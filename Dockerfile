...
RUN apt-get -y update && \
    mkdir -p /tmp/jho && \
    apt-get install -y lsb-release sudo vim curl wget git 

RUN echo "Targetplatform is ${TARGETPLATFORM}"

RUN cd /tmp/jho && \
    wget https://github.com/gohugoio/hugo/releases/download/v0.137.1/hugo_extended_0.137.1_linux-arm64.tar.gz && \
